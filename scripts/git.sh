...
IFS=' ' read -r -a hide_status <<< $(get_tmux_option "@orlock-git-disable-status" "false")
IFS=' ' read -r -a current_symbol <<< $(get_tmux_option "@orlock-git-show-current-symbol" "✓")
IFS=' ' read -r -a diff_symbol <<< $(get_tmux_option "@orlock-git-show-diff-symbol" "!")
IFS=' ' read -r -a no_repo_message <<< $(get_tmux_option "@orlock-git-no-repo-message" "")
...
# getting the #{pane_current_path} from orlock.sh is no longer possible
getPaneDir()
{
 nextone="false"
